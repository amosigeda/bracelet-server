...
public class HeartPressureServiceImpl implements IHeartPressureService {
	...
	public List<Map<String, Object>> monthData(Long user_id) {
		String sql = "SELECT DATE_FORMAT(upload_time,'%Y%m%d') day, max(max_heart_pressure) max, min(min_heart_pressure) min FROM heart_pressure WHERE user_id=? and DATE_FORMAT(upload_time,'%Y%m')=DATE_FORMAT(CURDATE(),'%Y%m') GROUP BY day order by day";
