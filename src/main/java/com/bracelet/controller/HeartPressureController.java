...
public class HeartPressureController extends BaseController {
	...
	/**
	 * 图表, 当月，1天2个数据点，当天最高，当天最低
	 */
	@ResponseBody
	@RequestMapping(value = "/chart/month/{token}", method = RequestMethod.GET)
	public HttpBaseDto listChartMonth(@PathVariable String token) {
		Long user_id = checkTokenAndUser(token);
		List<Map<String, Object>> monthData = heartPressureService.monthData(user_id);
		List<Map<String, Object>> dataList = new LinkedList<Map<String, Object>>();
		HttpBaseDto dto = new HttpBaseDto();
		for (Map<String, Object> row : monthData) {
			Map<String, Object> rowMap = new HashMap<>();
			rowMap.put("day", String.valueOf(row.get("day")));
			rowMap.put("max", String.valueOf(row.get("max")));
			rowMap.put("min", String.valueOf(row.get("min")));
			dataList.add(rowMap);
		}
