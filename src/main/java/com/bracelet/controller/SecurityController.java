...
public class SecurityController extends BaseController {
	...
	public HttpBaseDto getFence(@PathVariable String token) {
		...
		List<Fence> list = this.fenceService.getRoundFenceList(user_id);
		HttpBaseDto dto = new HttpBaseDto();
		List<Map<String, Object>> fenceDatalist = new LinkedList<Map<String, Object>>();
		if (list!=null&&list.size() > 0) {
			for (Fence fence : list) {
				...
				dataMap.put("name", fence.getName());
				fenceDatalist.add(dataMap);
			}
			// fencelog
			/*
			 * List<Fencelog> fencelogList =
			 * this.fencelogService.findByCount(user_id, 2, 3); if (fencelogList
			 * != null) { for (Fencelog fencelog : fencelogList) { Map<String,
			 * Object> fencelogDataMap = new HashMap<>();
			 ...
			 * fencelogDataMap.put("timestamp",
			 * fencelog.getUpload_time().getTime());
			 * fencelogDatalist.add(fencelogDataMap); } }
			 */
		//	dataMapp.put("fencelist", fenceDatalist);
		}
		dto.setData(fenceDatalist);
		return dto;
	}
	...
	public HttpBaseDto saveFence(@RequestParam String token, @RequestParam String lat, @RequestParam String lng,
			@RequestParam Integer radius,@RequestParam String name) {
		if (StringUtils.isAllEmpty(lat, lng,name)) {
		...
		Long user_id = checkTokenAndUser(token);
		if (this.fenceService.insert(user_id, lat, lng, radius,name)) {
			HttpBaseDto dto = new HttpBaseDto();
			return dto;
		} else {
	...
	public HttpBaseDto updateFence(@RequestParam String token, @RequestParam Long id, @RequestParam String lat,
			@RequestParam String lng, @RequestParam Integer radius,@RequestParam String name) {
		if (StringUtils.isAllEmpty(lat, lng,name)) {
			throw new BizException(RespCode.NOTEXIST_PARAM);
		}
		...
		Long user_id = checkTokenAndUser(token);
		if (this.fenceService.update(id, user_id, lat, lng, radius,name)) {
			HttpBaseDto dto = new HttpBaseDto();
			return dto;
		} else {
	...
	public HttpBaseDto listSensitivePoint(@PathVariable String token) {
		Long user_id = checkTokenAndUser(token);
		List<SensitiveArea> splist = this.sensitivePointService.find(user_id);
		...
		if (splist != null && splist.size() > 0) {
			for (SensitiveArea sp : splist) {
				...
				dataMap.put("timestamp", sp.getUploadtime().getTime());
	...
	public HttpBaseDto saveSensitivePoint(@RequestParam String token, @RequestParam String name,
			@RequestParam String lat, @RequestParam String lng, @RequestParam Integer radius) {
		if (StringUtils.isAllEmpty(lat, lng,name)) {
			throw new BizException(RespCode.NOTEXIST_PARAM);
		}
		if (radius == null || radius < 0) {
		...
		Long user_id = checkTokenAndUser(token);
		if (this.sensitivePointService.insertSensitiveArea(user_id, name, lat, lng, radius)) {
			HttpBaseDto dto = new HttpBaseDto();
			return dto;
		} else {
	...
	public HttpBaseDto updateSensitivePoint(@RequestParam String token, @RequestParam Long id,
			@RequestParam String name, @RequestParam String lat, @RequestParam String lng,
		...
		Long user_id = checkTokenAndUser(token);
		if (this.sensitivePointService.updateSensitiveArea(id, user_id, name, lat, lng, radius)) {
			HttpBaseDto dto = new HttpBaseDto();
			return dto;
		} else {
	...
	public HttpBaseDto deleteSensitivePoint(@RequestParam String token, @RequestParam Long id) {
		...
		Long user_id = checkTokenAndUser(token);
		if (this.sensitivePointService.deleteSensitiveArea(id, user_id)) {
			HttpBaseDto dto = new HttpBaseDto();
			return dto;
		} else {
	...
	public HttpBaseDto getOddShapeFence(@PathVariable String token) {
		Long user_id = checkTokenAndUser(token);
		List<Map<String, Object>> dataList = new LinkedList<Map<String, Object>>();
		List<OddShape> list = this.fenceService.getOddShapeList(user_id);
		if (list != null) {
			for (OddShape ood : list) {
				Map<String, Object> dataMap = new HashMap<>();
				dataMap.put("id", ood.getId());
				dataMap.put("point", ood.getPoint());
				dataMap.put("name", ood.getName());
				dataMap.put("type", ood.getType());
				dataMap.put("timestamp", ood.getCreatetime().getTime());
				dataList.add(dataMap);
			}
		}
		HttpBaseDto dto = new HttpBaseDto();
	...
	public HttpBaseDto saveOtherFence(@RequestParam String token, @RequestParam String point,
		...
		Long user_id = checkTokenAndUser(token);
		if (this.fenceService.insertOddShape(user_id, point, name, "1")) {
			HttpBaseDto dto = new HttpBaseDto();
			return dto;
		} else {
	...
	@ResponseBody
	@RequestMapping(value = "/OddShapefence/update", method = RequestMethod.POST)
	public HttpBaseDto updateOddShapefence(@RequestParam String token, @RequestParam Long id,
			@RequestParam String point, @RequestParam String name) {
		if (StringUtils.isAllEmpty(point)) {
			throw new BizException(RespCode.NOTEXIST_PARAM);
		}
		char[] chars = point.toCharArray();
		...
		Long user_id = checkTokenAndUser(token);
		if (this.fenceService.updateOddShapefence(id, user_id, point, name)) {
			HttpBaseDto dto = new HttpBaseDto();
			return dto;
		} else {
			logger.info("用户修改异形电子围栏失败, token:" + token + ",userId:" + user_id + ",id:" + id + ",point:" + point);
			throw new BizException(RespCode.SYS_ERR);
		}
	}
